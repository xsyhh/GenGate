...
from .mc_two_stage import compute_task_mean_targets, materialize_pre_mc_targets, train_probe_soft_targets
from .metrics import (
    attach_expert,
    cascade_grid,
    load_expert_map,
...
from .probe import load_probe, predict_self_probs, resolve_device, save_probe


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train+evaluate MC Two-Stage Probe with pre/post probes.")
    ...
    p.add_argument("--no_dedupe_pre_tasks", action="store_true")
    return p.parse_args()
...
def main() -> None:
    args = parse_args()
    paths = make_run_paths(method="MC-Two-Stage-Probe", domain=args.domain, model=args.model, run_name=args.run_name)
    ...
    if args.pre_probe and args.post_probe:
        ...
        train_pre_x, _, train_pre_meta = train_source.materialize_ratio(0.0)
        train_post_x, train_post_y, train_post_meta = train_source.materialize_ratio(1.0)

        post_probe = train_probe_soft_targets(
            train_post_x,
            train_post_y.to(dtype=train_pre_x.dtype),
            epochs=args.epochs,
        ...
        task_targets = compute_task_mean_targets(train_post_meta, train_post_y)
        train_pre_x_mc, train_pre_y_mc, _ = materialize_pre_mc_targets(
            train_pre_x,
            train_pre_meta,
            task_targets,
            dedupe_by_task=(not args.no_dedupe_pre_tasks),
        )
        pre_probe = train_probe_soft_targets(
            train_pre_x_mc,
            train_pre_y_mc,
            epochs=args.epochs,
            batch_size=args.batch_size,
        ...
        pre_cfg = {
            ...
            "lr": args.lr,
            "seed": args.seed,
            "target_type": "task_mean_post_label",
            "loss_type": "local_pref_bce_margin_no_kl",
            "dedupe_by_task": bool(not args.no_dedupe_pre_tasks),
            "num_pre_train_rows": int(train_pre_x_mc.shape[0]),
        }
        post_cfg = {
            "method": "MC-Two-Stage-Probe",
            "stage": "post",
        ...
        json.dump(
            {
                ...
                "pre_threshold": pre_t,
                "post_threshold": post_t,
                "pre_target": "task_mean_post_label",
                "post_target": "sample_label",
                "dedupe_pre_tasks": bool(not args.no_dedupe_pre_tasks),
            },
            f,
            ensure_ascii=False,
            indent=2,
