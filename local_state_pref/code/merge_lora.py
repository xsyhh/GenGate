...
def resolve_base_model_path(lora_ckpt: str, base_model: str | None) -> str:
    ...
    if not inferred:
        raise ValueError("Unable to infer base model from adapter_config.json; pass --base_model explicitly.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Merge a code LoRA checkpoint into its base model for vLLM generation.")
