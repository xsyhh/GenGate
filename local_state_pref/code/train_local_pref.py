...
from common import ATTEMPT_CLOSE
...
def _extract_kl_targets(
    ...
    suffix = code_text + ATTEMPT_CLOSE
    if not context.endswith(suffix):
        raise ValueError("S1 context does not end with code text followed by attempt close marker")

    prefix_text = context[: -len(suffix)]
    prefix_ids = tokenizer(prefix_text, add_special_tokens=False)["input_ids"]
    prefix_plus_code_ids = tokenizer(prefix_text + code_text, add_special_tokens=False)["input_ids"]
    if prefix_plus_code_ids[: len(prefix_ids)] != prefix_ids:
        raise ValueError("Prefix tokenization is not a prefix of prefix+code tokenization")

    full_kl_target_ids = prefix_plus_code_ids[len(prefix_ids) :]
...
def prepare_pair_example(tokenizer, record: dict[str, Any], max_seq_length: int) -> dict[str, Any]:
    ...
    action_a_ids = extract_action_suffix_ids(tokenizer, context, str(record["action_a"]))
    action_b_ids = extract_action_suffix_ids(tokenizer, context, str(record["action_b"]))
...
def _compute_selected_kl(
    ...
    tau = float(temperature)
    student_sel = student_logits.index_select(1, selected_positions) / tau
    ref_sel = ref_logits.index_select(1, selected_positions) / tau
    student_log_probs = F.log_softmax(student_sel, dim=-1)
    ref_probs = F.softmax(ref_sel, dim=-1)
    return F.kl_div(student_log_probs, ref_probs, reduction="batchmean", log_target=False) * (tau ** 2)


def _compute_s1_code_kl(
    model,
    ref_model,
    input_ids: torch.Tensor,
    attention_mask: torch.Tensor,
...
def validate_resume_checkpoint_compatible(
    checkpoint: str | None,
    training_args,
    *,
    allow_batch_size_mismatch: bool = False,
) -> None:
    if not checkpoint or allow_batch_size_mismatch:
        return
    ...
    if os.path.isfile(state_path):
        with open(state_path, "r", encoding="utf-8") as f:
            state = json.load(f)
        state_train_batch_size = state.get("train_batch_size")
        if state_train_batch_size is not None and int(state_train_batch_size) != int(training_args.train_batch_size):
            raise ValueError(
                "Unsafe resume checkpoint: trainer_state.json records train_batch_size="
                f"{state_train_batch_size}, but current TrainingArguments.train_batch_size="
                f"{training_args.train_batch_size}. HuggingFace Trainer will override the current batch size "
                "with the checkpoint value and silently change the step schedule. For true resume, keep the "
                "same per_device_train_batch_size/gradient_accumulation_steps as the checkpoint. To continue "
                "from LoRA weights with different batch settings, start a new run instead of Trainer resume."
            )
    ...
    checkpoint_args = torch.load(training_args_path, map_location="cpu", weights_only=False)
    mismatches = []
    for key in ("per_device_train_batch_size", "gradient_accumulation_steps"):
        old_value = getattr(checkpoint_args, key, None)
        new_value = getattr(training_args, key, None)
        if old_value is not None and new_value is not None and int(old_value) != int(new_value):
            mismatches.append(f"{key}: checkpoint={old_value}, current={new_value}")

    if mismatches:
        raise ValueError(
            "Unsafe resume checkpoint: batch schedule differs from checkpoint training_args.bin ("
            + "; ".join(mismatches)
            + "). For true resume, keep these values unchanged. To change them, start a new training run "
            "from model/LoRA weights without Trainer optimizer/scheduler resume."
        )


def build_dataset_map_kwargs(
    *,
    desc: str,
    tokenize_num_proc: int,
    tokenize_batch_size: int,
    tokenized_cache_file: str | None,
) -> dict[str, Any]:
    kwargs: dict[str, Any] = {
        "desc": desc,
        "writer_batch_size": tokenize_batch_size,
    }
    if tokenize_num_proc > 1:
        kwargs["num_proc"] = tokenize_num_proc
    if tokenized_cache_file:
        kwargs["cache_file_name"] = tokenized_cache_file
    return kwargs
...
def main():
    ...
    class LocalPrefTrainer(Trainer):
        def __init__(
            self,
            loss_type: str,
            beta: float,
        ...
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            ...
            if self.loss_type == "dpo":
                policy_a, policy_b = _batch_action_pair_logprobs(
                    model=model,
                    input_ids=input_ids,
            ...
            if self.loss_type == "dpo":
                with torch.no_grad():
                    ref_a, ref_b = _batch_action_pair_logprobs(
                ...
                margin = self.beta * (margin - (ref_a - ref_b))
            ...
            if self.loss_type == "dpo" and self.ref_model is not None and self.kl_weight > 0:
                kl_loss = _compute_s1_code_kl(
                    model=model,
                    ref_model=self.ref_model,
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    kl_target_ids=inputs["kl_target_ids"],
                    kl_target_mask=inputs["kl_target_mask"],
                    kl_target_start=inputs["kl_target_start"],
                    is_s1=is_s1,
                    temperature=self.kl_temperature,
                )
            if self.kl_weight > 0 and kl_loss is not None:
                loss = loss + self.kl_weight * kl_loss

            return (loss, None) if return_outputs else loss

    parser = argparse.ArgumentParser(description="Train local same-state preference policy for code routing.")
    parser.add_argument("--model_path", required=True)
    parser.add_argument("--dataset_path", required=True)
    parser.add_argument("--output_dir", required=True)
    parser.add_argument("--loss_type", choices=["bce", "dpo"], default="bce")
    ...
    parser.add_argument("--allow_resume_batch_size_mismatch", action="store_true", default=False)
    parser.add_argument("--tokenize_num_proc", type=int, default=8)
    parser.add_argument("--tokenize_batch_size", type=int, default=1000)
    parser.add_argument("--tokenized_cache_file", default=None)
    ...
    tokenized = raw_dataset.map(
        ...
        **build_dataset_map_kwargs(
            desc="Tokenizing pair records",
            tokenize_num_proc=args.tokenize_num_proc,
            tokenize_batch_size=args.tokenize_batch_size,
            tokenized_cache_file=args.tokenized_cache_file,
        ),
    )
    ...
    ref_model = None
    if args.loss_type == "dpo" or args.kl_weight > 0:
    ...
    train_kwargs = build_trainer_train_kwargs(args.resume_from_checkpoint, output_dir=args.output_dir)
    validate_resume_checkpoint_compatible(
        train_kwargs.get("resume_from_checkpoint"),
        training_args,
        allow_batch_size_mismatch=args.allow_resume_batch_size_mismatch,
    )
