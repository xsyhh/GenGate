...
from io_utils import dump_jsonl, load_jsonl, read_math_rows
...
def _build_first_rollout_success_map(evaluated_rows: list[dict]) -> dict[str, float]:
    ...
    for row in evaluated_rows:
        ...
        if not task_id or not raw_output or not extracted:
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Build MATH same-state BCE preference pairs from evaluated rollouts.")
    parser.add_argument("--evaluated", required=True, help="JSONL output from math_attempt/step2_evaluate.py")
    parser.add_argument("--data_jsonl", required=True, help="Original MATH train/eval JSONL")
    ...
    raw_map = read_math_rows(
        args.data_jsonl,
        task_id_key=args.task_id_key,
        problem_key=args.problem_key,
    )
    evaluated_rows = load_jsonl(args.evaluated)
    records = build_pair_records(raw_map, evaluated_rows)
    first_rollout_success = _build_first_rollout_success_map(evaluated_rows)
    ...
    print(f"Saved {len(records)} MATH pair records to: {args.output}")
    print(
